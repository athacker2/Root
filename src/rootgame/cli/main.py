...
def main():
    ...
    while(game.round < 10):
        ...
        while(game.round == curr_round):
            print(f"{game.current_player.faction.faction_name}'s turn. Current phase: {game.current_phase.name}")
            ...
            if(game.current_player.faction.faction_name == "eyrie_dynasties"):
                print(f"Current Decree:")
                print(f"Recruit: {[card.suit.__str__() for card in game.current_player.faction.get_remaining_decree(DecreeOption.Recruit)]}")
                print(f"Move: {[card.suit.__str__() for card in game.current_player.faction.get_remaining_decree(DecreeOption.Move)]}")
                print(f"Battle: {[card.suit.__str__() for card in game.current_player.faction.get_remaining_decree(DecreeOption.Battle)]}")
                print(f"Build: {[card.suit.__str__() for card in game.current_player.faction.get_remaining_decree(DecreeOption.Build)]}")
                print()

            legal_actions = game.get_legal_actions()
            print(f"Legal Actions: - {game.current_player.faction.faction_name}")
            for(j, action) in enumerate(legal_actions):
                print(f"{j}: {action}")

            chosen_action = None
            while(not(game.is_action_legal(chosen_action))):
                chosen_action = input(f"Choose an action: ")
